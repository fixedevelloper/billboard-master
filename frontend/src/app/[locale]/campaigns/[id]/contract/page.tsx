...
export default function CampaignContractPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const { data: billboard } = useSWR(
    ...
    ([, billboardId]) => getBillboard(billboardId),
  );
  const {
    data: contract,
    error: contractError,
  ...
  async function handleCreate(event: FormEvent) {
    ...
    if (!booking || !billboard || !campaign) return;
    setSubmitting(true);
    setError(null);
    try {
